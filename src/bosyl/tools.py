...
def get_syls(text):
    ...
    syllable_tokens = tokenizer.tokenize()
...
def parse_syl(syl_text):
    ...
    yang_jug = ''
    sc = SylComponents()
    components = sc.get_parts(syl_text)
    if components:
...
def get_bo_symbols(text):
    bo_symbols = []
    syl_tokens = get_syls(text)
    for syl_type,syl_text in syl_tokens:
        has_tseg = False
        if syl_type == 'TEXT':
            if '་' in syl_text:
                syl_text= syl_text[:-1]
                has_tseg = True
            syl_text = syl_text.replace('་', '')
            syl_parts = parse_syl(syl_text)
            bo_symbols += [symbol for _, symbol in syl_parts.items() if symbol]
            if has_tseg:
                bo_symbols.append('་')
        elif syl_type == 'PUNCT':
            bo_symbols.append(syl_text)
    return bo_symbols
